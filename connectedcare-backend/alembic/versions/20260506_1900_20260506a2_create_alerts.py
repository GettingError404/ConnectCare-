...
def upgrade() -> None:
    op.create_unique_constraint("uq_health_vitals_id", "health_vitals", ["id"])
    op.create_table(
        "alerts",
        ...
        sa.ForeignKeyConstraint(["vital_id"], ["health_vitals.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("user_id", "vital_id", "alert_type", name="uq_alerts_user_vital_type"),
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint("uq_health_vitals_id", "health_vitals", type_="unique")
